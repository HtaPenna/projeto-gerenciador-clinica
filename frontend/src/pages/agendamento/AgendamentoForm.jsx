...
export default function NovoEventoModal({ isOpen, onRequestClose, dataSelecionada, onSuccess, dentistaId }) {
  ...
  useEffect(() => {
    if (status === "bloqueado") {
      ...
      setTitulo(pacienteSelecionado.nome);
    } else {
      setTitulo("");
    }
  ...
  useEffect(() => {
    if (pacienteInput.length > 0 && status !== "bloqueado") {
  ...
  const handleSubmit = async (e) => {
    ...
    // Paciente obrigatório apenas se não for bloqueado
    if (status !== "bloqueado" && !pacienteSelecionado) {
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {/* Paciente autocomplete - só mostra se não for bloqueado */}
        {status !== "bloqueado" && (
